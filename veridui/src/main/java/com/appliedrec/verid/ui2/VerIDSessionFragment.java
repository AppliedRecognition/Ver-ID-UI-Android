 ...
 * @since 1.0.0
 */
public class VerIDSessionFragment extends Fragment implements BiConsumer<FaceDetectionResult,String> {
     ...
     */
    @UiThread
    public Optional<PreviewView> getViewFinder() {
        if (viewBinding == null) {
            return Optional.empty();
        }
        return Optional.of(viewBinding.viewFinder);
    }
    ...
    public void accept(FaceDetectionResult faceDetectionResult, String s) {
        ...
        RectF defaultFaceBounds = faceDetectionResult.getDefaultFaceBounds();
        ...
        try {
            float scale = Math.max((float)viewBinding.viewFinder.getWidth() / (float)faceDetectionResult.getImageSize().width, (float)viewBinding.viewFinder.getHeight() / (float)faceDetectionResult.getImageSize().height);
            faceBoundsMatrix.reset();
            faceBoundsMatrix.setScale(scale, scale);
            faceBoundsMatrix.postTranslate((float)viewBinding.viewFinder.getWidth() / 2f - (float)faceDetectionResult.getImageSize().width * scale / 2f, (float)viewBinding.viewFinder.getHeight() / 2f - (float)faceDetectionResult.getImageSize().height * scale / 2f);
            ...
            if (plotFaceLandmarks && faceDetectionResult.getFaceLandmarks() != null && faceDetectionResult.getFaceLandmarks().length > 0 && Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
