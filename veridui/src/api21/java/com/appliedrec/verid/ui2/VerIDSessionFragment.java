...
public class VerIDSessionFragment extends Fragment implements IVerIDSessionFragment2, TextureView.SurfaceTextureListener, IVideoRecorder, FlowableOnSubscribe<VerIDImage> {
    ...
    private FlowableEmitter<VerIDImage> imageEmitter;
    ...
    private final Object imageEmitterLock = new Object();
    ...
    public void drawFaceFromResult(FaceDetectionResult faceDetectionResult, VerIDSessionResult sessionResult, RectF defaultFaceBounds, @Nullable EulerAngle offsetAngleFromBearing, String labelText) {
        ...
        if (sessionSettings != null && sessionResult.getFaceCaptures().length >= sessionSettings.getNumberOfFacesToCapture()) {
...
//        configureTransform(width, height);
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        if (imageProcessingExecutor.getActiveCount() == 0 && imageProcessingExecutor.getQueue().isEmpty()) {
            ...
            imageProcessingExecutor.execute(() -> {
                ...
                synchronized (imageEmitterLock) {
                    if (imageEmitter != null && !imageEmitter.isCancelled()) {
                        imageEmitter.onNext(verIDImage);
                    }
                }
            });
        }
     ...
     * @since 1.0.0
     */
    public Matrix imageScaleTransformAtImageSize(com.appliedrec.verid.core2.Size size) {
            ...
            cameraDevice.createCaptureSession(surfaces,
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigureFailed(@NonNull CameraCaptureSession session) {
                            ...
                            synchronized (imageEmitterLock) {
                                if (imageEmitter != null && !imageEmitter.isCancelled()) {
                                    imageEmitter.onError(sessionException);
                                }
                            }
                        }
                    }, backgroundHandler);
    ...
    @Override
    public void subscribe(FlowableEmitter<VerIDImage> emitter) throws Exception {
        synchronized (imageEmitterLock) {
            imageEmitter = emitter;
            imageEmitter.setCancellable(this::closeCamera);
        }
    }
}
